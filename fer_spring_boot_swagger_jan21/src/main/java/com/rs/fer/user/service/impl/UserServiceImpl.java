...
public class UserServiceImpl implements UserService {
	 ...
	 * response = null; Optional<User> userObj = userRepository.findById(userid); if
	 * (userObj.isPresent()) { response = new
	 * GetUserResponse(HttpStatus.PRECONDITION_FAILED, "001",
	 * "User is already registered with the given email address", null); return
	 * response; } else { response = new
	 * GetUserResponse(HttpStatus.INTERNAL_SERVER_ERROR, "002",
	 * "User registration is failed", null); }
	 * 
	 * return response; }
	 */
